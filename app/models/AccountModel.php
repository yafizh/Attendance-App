...
class AccountModel
{
    ...
    public function putPassword()
    {
        ...
        $employee_nip = $_SESSION['login'];

        $query = "SELECT * FROM " . $this->employee_table . " WHERE employee_nip = '" . $employee_nip . "'";

        $this->db->query($query);
        $this->db->execute();

        if ($employee_nip == 'admin') {
            ...
            if ($old_password == $this->db->single()['employee_password']) {
                if ($new_password == $confirm_new_password) {
                    $query = "UPDATE " . $this->employee_table . " SET employee_password = '" . $new_password . "' WHERE employee_nip = '" . $employee_nip . "'";
                    $this->db->query($query);
                    $this->db->execute();
                } else return false;
            } else return false;
